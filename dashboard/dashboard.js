...
function doGet() {
      ...
      data.forEach(row => {
        ...
        if (dispatcher && dispatcher.toString().trim() !== '') {
          ...
          workPerformanceMap[dispatcherLower] = {
            ...
            performanceScore: row[50] || 0,  // AY열: 성과 점수
            trend: [
              row[45] || 0,  // AT열: 추세 데이터 1
              row[46] || 0,  // AU열: 추세 데이터 2
              row[47] || 0,  // AV열: 추세 데이터 3
              row[48] || 0,  // AW열: 추세 데이터 4
              row[49] || 0   // AX열: 추세 데이터 5
            ]
          };
        }
      });
  ...
  function getRegionStats() {
    try {
      ...
      // 지역 통계 배열로 변환
      const regionStats = Object.keys(regionCounts).map(region => {
        return {
          region: region,
  ...
  function getDispatchData() {
    // 배차 데이터가 없으면 빈 객체 반환
    if (!rawData || !rawData.length) {
      console.warn('배차 데이터가 없습니다.');
      return { data: [], headers: [] };
    }
    
    console.log('배차 데이터 처리 시작:', rawData.length + '개 항목');
  ...
  async function loadData(showSpinner = true, forceWorkPerformanceRefresh = false) {
    ...
    try {
      ...
      // 원시 데이터 가져오기
      const getRawData = new Promise((resolveRaw) => {
        google.script.run
          .withSuccessHandler((data) => {
            console.log('원시 데이터 로드 성공:', data ? data.length + '개 항목' : '데이터 없음');
            rawData = data || []; // 전역 변수에 원시 데이터 저장
            resolveRaw(data);
          })
          .withFailureHandler((error) => {
            console.error('원시 데이터 로드 오류:', error);
            rawData = []; // 오류 시 빈 배열로 초기화
            resolveRaw(null);
          })
          .getDispatchData();
      });
      ...
      // 모든 데이터를 한 번에 로드
      Promise.all([getRawData, getSummaryData, getDispatcherData, getHourlyData, getPerformersData, loadWorkPerformanceData])
        .then(([rawDataResult, summary, dispatcherStats, hourlyStats, performers]) => {
